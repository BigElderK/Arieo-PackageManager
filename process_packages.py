...
Examples:
  ...
  python process_packages.py --env{ARIEO_PACKAGE_BUILD_HOST_PRESET}=windows.x86_64 --env{ARIEO_PACKAGE_BUILD_TYPE}=Release
  
  # Build with multi-value environment variables (builds all combinations)
  python process_packages.py --env{ARIEO_PACKAGE_BUILD_HOST_PRESET}=[windows.x86_64, ubuntu.x86_64] --env{ARIEO_PACKAGE_BUILD_TYPE}=[Release, Debug]
  
  # Build with repeated environment variables (builds all combinations)
  python process_packages.py --env{ARIEO_PACKAGE_BUILD_HOST_PRESET}=windows.x86_64 --env{ARIEO_PACKAGE_BUILD_HOST_PRESET}=ubuntu.x86_64 --env{ARIEO_PACKAGE_BUILD_TYPE}=Release --env{ARIEO_PACKAGE_BUILD_TYPE}=Debug
  
  # Combine package filter and environment variables
  python process_packages.py --package=Arieo-BuildEnv --env{ARIEO_PACKAGE_BUILD_HOST_PRESET}=windows.x86_64
        ''',
